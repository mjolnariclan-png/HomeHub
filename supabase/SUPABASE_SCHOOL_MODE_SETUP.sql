...
create policy "school_mode_read_linked_student" on public.school_mode_schedules for select to authenticated using (
  ...
  or exists (select 1 from public.students s join public.profiles p on p.family_id = s.family_id where s.id = student_id and p.id = auth.uid() and p.role in ('admin', 'parent', 'adult'))
);
create policy "school_mode_manage_adults" on public.school_mode_schedules for all to authenticated using (
  exists (select 1 from public.students s join public.profiles p on p.family_id = s.family_id where s.id = student_id and p.id = auth.uid() and p.role in ('admin', 'parent', 'adult'))
) with check (
  exists (select 1 from public.students s join public.profiles p on p.family_id = s.family_id where s.id = student_id and p.id = auth.uid() and p.role in ('admin', 'parent', 'adult'))
);
