...
async function renderMessages(container) {
    ...
    container.innerHTML = `<div class="fade-in"><div class="card"><div class="card-header"><div class="card-title">💬 Family Messages</div>${store.messagingTransportReady ? '<button class="btn btn-ghost btn-sm" onclick="initializeMessaging()">Connect Device</button>' : ''}</div>${store.messagingTransportReady ? `<div class="list-container" style="margin-bottom:16px;">${messages.map((message) => `<div class="list-item"><div class="list-content"><div class="list-title">${message.senderName}</div><div>${message.text}</div><div class="list-meta"><span>${new Date(message.sentAt).toLocaleString()}</span></div></div></div>`).join('') || '<div class="empty-state-small">No messages stored on this device.</div>'}</div><div style="display:flex;gap:8px;align-items:center;"><input class="form-input" id="messageText" placeholder="Message your family" style="flex:1;"><button class="btn btn-primary" onclick="sendFamilyMessage()">Send</button></div>` : '<div style="color:var(--text-muted);">Run SUPABASE_MESSAGING_TRANSPORT.sql in Supabase, then refresh.</div>'}</div></div>`;
}
